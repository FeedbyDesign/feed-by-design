...
class ContactForm extends Component {
  ...
  handleSubmit(e) {
    ...
    fetch(this.props.location.pathname, {
      method: "POST",
      headers: myHeaders,
      body: "form-name=" + this.props.formSettings.form.name + "&botField=" + this.state.botField + "&name=" + this.state.name + "&phone=" + this.state.phone + "&email=" + this.state.email + "&message=" + this.state.message
    })
    .then((res) => {
      if (res.status >= 400) {
